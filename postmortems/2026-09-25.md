...
- **Duration:** 09:29 Central to 10:30 Central (1 hour)
...
### Contributing factor
The change took place inside a large refactor, which made it easier for the lost WebSocket behavior to blend into the surrounding changes and harder to spot during review. In the future, work should be scoped into smaller chunks so regressions like this are easier to catch visually, too.
...
| # | Action | Owner | Status |
|---|--------|-------|--------|
| 1 | Register supported subprotocols with `.protocols(...)` and use the client's protocol | Jacob Curley | Done |
| 2 | **Add integration tests to the CI/CD pipeline** that start the server and open real WebSocket connections to the subscription endpoints (see plan below) | Software Group B | Planned |
| 3 | Check the `connection_init` payload shape the Flutter clients send (`Authorization` vs `authorization` vs a nested `headers` object) and handle each case | Software Group B | Planned |
